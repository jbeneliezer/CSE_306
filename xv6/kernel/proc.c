...
wakeup1(void *chan)
{
  ...
  for (p = ptable.proc; p < &ptable.proc[NPROC]; p++)
    if (p->state == SLEEPING && p->chan == chan)
    {
      ...
      struct rtcdate *r;
      cmostime(r);
      p->pt_sleep += timediff(r, p->bedtime);
      sched();
    }
}
...
found:
  ...
  *(uint *)sp = (uint)trapret;
  ...
  // Copy process state from proc.
  if ((p->pgdir = copyuvm(initproc->pgdir, initproc->sz)) == 0)
  {
    kfree(p->kstack);
    p->kstack = 0;
    p->state = UNUSED;
    cprintf("Error copying pgdir.\n");
    return;
  }
  p->sz = initproc->sz;
  p->parent = initproc;
  *p->tf = *initproc->tf;
  ...
  for (i = 0; i < NOFILE; i++)
    if (initproc->ofile[i])
      p->ofile[i] = filedup(initproc->ofile[i]);
  p->cwd = idup(initproc->cwd);

  safestrcpy(p->name, initproc->name, sizeof(initproc->name));
...
int sigsend(void)
{
  ...
  if (argint(0, (int*)&pid) < 0 || argint(1, &sig) < 0) return -1;
  for (p = ptable.proc; p < &ptable.proc[NPROC]; ++p)
  {
    if (p->pid == pid)
    {
      if (p->state == SLEEPING)
        p->state = RUNNABLE;  // wake if sleeping
      release(&ptable.lock);
      if (p->sigptr == &p->sigs[NSIGS])
      {
        cprintf("Signal array full\n");
        return -1;
      }
      else
      {
...
int sigsethandler()
{
  ...
  if (argint(0, &sig) < 0 || argint(1, &tmp) < 0) return -1;
  hand = (void*) tmp;
  ...
  for (s = p->sigs; s < p->sigptr; ++s)
  {
    if (s->n == sig)
    {
      ...
      else if (s->n == -2)
      {
        struct signal *s2 = s;
        ++s2;
        memmove(s, s2, (p->sigptr - s)/sizeof(s));
      }
      else
        s->handler = hand;
    }
...
int siggetmask(void)
{
  return myproc()->mask.n;
}

int sigsetmask(void)
{
  int maskp;
  if (argint(0, &maskp) < 0) return -1;
  myproc()->mask.n = maskp;
  return maskp;
}
